...
/// </summary>
public class UnityMusic : MonoBehaviour
{
	...
	private static UnityMusic Current;
	private static List<UnityMusic> MusicList = new List<UnityMusic>();
	...
	public static void Play( string MusicName ) { 
		UnityMusic newMusic = MusicList.Find( ( UnityMusic m ) => m.name == MusicName );
		if ( newMusic != null )
		{
			newMusic.PlayStart();
		}
		...
		{
			Debug.LogWarning( "Can't find music name : " + MusicName );
		}
	}
	/// <summary>
	...
	{
		Debug.Log( "SetNextBlock : "+blockName );
		Current.NextBlockName = blockName;
	}
	public static string GetNextBlockName() { return Current.NextBlockName; }
	public static string GetCurrentBlockName() { return Current.name; }
#endif
	...
	void Update()
	{
		...
		{
			if ( NextBlockName != "" && NumBar > 0 && Now_.totalUnit >= 0 )
			{
				UnityMusic.Play( NextBlockName );
				NextBlockName = "";
			}
			return;
		}
