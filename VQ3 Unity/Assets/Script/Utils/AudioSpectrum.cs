...
public class AudioSpectrum : MonoBehaviour
{
    ...
    public float accentThreashold = 0.3f;
    ...
    public float[] rawSpectrum { get; private set; }
    public float[] rawSpectrumRight { get; private set; }
    public float[] rawSpectrumLeft { get; private set; }

    public bool isNowAccent { get; private set; }
    public bool isJustAccent { get; private set; }
    #endregion
    ...
    void CheckBuffers ()
    {
        if (rawSpectrum == null || rawSpectrum.Length != numberOfSamples) {
            ...
            rawSpectrumRight = new float[numberOfSamples];
            rawSpectrumLeft = new float[numberOfSamples];
        }
    ...
    void Update ()
    {
        ...
        AudioListener.GetSpectrumData(rawSpectrumRight, 0, fftWindow);
        AudioListener.GetSpectrumData( rawSpectrumLeft, 1, fftWindow );
        for( int i=0; i<rawSpectrum.Length; ++i )
        {
            rawSpectrum[i] = (rawSpectrumRight[i] + rawSpectrumLeft[i]) / 2;
        }
        ...
        if (Music.isNowChanged)
        {
            ...
            {
                if (rawSpectrum[i] > accentThreashold)
                {
                    isNowAccent = true;
                    break;
                }
