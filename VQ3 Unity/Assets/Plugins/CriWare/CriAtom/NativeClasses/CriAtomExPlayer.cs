...
public class CriAtomExPlayer : IDisposable
{
	 ...
	 * CriAtomExPlayback playback = playerStart();
	 * 	：
	 * // パンニング3Dボリュームの変更
	...
	{
		criAtomExPlayer_SetAisacByName(this.handle, controlName, value);
	}
	...
	{
		criAtomExPlayer_SetAisacById(this.handle, (ushort)controlId, value);
	}
	...
	[DllImport(CriWare.pluginName)]
	private static extern void criAtomExPlayer_SetAisacById(
		IntPtr player, ushort control_id, float control_value);
	
	[DllImport(CriWare.pluginName)]
	private static extern void criAtomExPlayer_SetAisacByName(
